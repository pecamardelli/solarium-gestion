...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			function actualizar(){
				...
				var estado = document.getElementById('estadoUser').value;
	...
	<body id="body">
		...
		<?php
			...
			$query="SELECT  userName AS us,
							usuarioNombre AS nom,
			...
			echo "<div>
				<center>
					<table id='table1'>
						<tr>
							<td colspan=2 style='font-weight: bold;'>DATOS DE " . strtoupper($fila['nom']) . " " . strtoupper($fila['ap']) . "</td>
						...
						<tr>
							<td>Rol</td>
							<td>
								<select id='rolUser' class='inputs' style='width: 50px;'>";
								for($i=1;$i<=5;$i++){
									if($_SESSION['rol'] <= $i){
										echo "<option value='$i'";
										if($fila['rol'] == $i){
						...
						<tr>
							<td>Estado</td>
							<td style='width: 190px;'>
								<select id='estadoUser' style='width: 120px;' class='inputs'>";
									$sql = "SHOW COLUMNS FROM `usuarios` LIKE 'estado'";
						...
						<tr>
							<td colspan=2><input type='button' value='Guardar' class='boton2' onClick='actualizar()'></td>
						</tr>
					</table>
				</center>
			</div>";
