<?php    
	...
	for($i=30;$i>=1;$i--){
		...
		#$query="SELECT DATE(fechaVenta) AS fecha, COUNT(*) AS cantidad FROM ventas WHERE fechaVenta >= '$dia 00:00:00' AND fechaVenta <= '$dia 23:59:59' GROUP BY fecha";
		$query="SELECT COUNT(*) AS cantidad FROM ventas WHERE fechaVenta >= '$dia 00:00:00' AND fechaVenta <= '$dia 23:59:59'";
		$result=mysqli_query($dbcon, $query) or die(mysqli_error($dbcon));
		$fila = mysqli_fetch_array($result, MYSQLI_ASSOC);
			
		if($fila['cantidad']){
			array_push($cants, $fila['cantidad']);
		}
		else{
			array_push($cants, "0");
		}
		
		$total_gral = (($total_gral + $fila['cantidad']));
	...
	$myPicture->drawText(200,35,"Cantidad de ventas",array("FontSize"=>20,"Align"=>TEXT_ALIGN_BOTTOMRIGHT)); 
