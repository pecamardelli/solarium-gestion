<?php    
	...
	while($fila = mysqli_fetch_array($result, MYSQLI_ASSOC)){
		$query="SELECT COUNT(*) AS cant FROM turnos WHERE idMaq='$fila[id]' AND start >= $start AND estado='TOMADO'";
		$result2 = mysqli_query($dbcon, $query) or die(mysqli_error($dbcon));
		$fila2 = mysqli_fetch_array($result2, MYSQLI_ASSOC);
		
		$percent = round((($fila2['cant']*100/720))*100)/100;
		
		array_push($cants, $percent);
		array_push($marcas, $fila['marca']);
		$total_gral = (($total_gral + $percent));
	}

	$promedio = round((($total_gral/mysqli_num_rows($result)))*100)/100;
	
	/* Create and populate the pData object */ 
	$MyData = new pData();   
	$MyData->addPoints($cants,"Uso Promedio " . $promedio . " %");
	...
	$myPicture->drawText(320,37,"Porcentaje de uso de las máquinas",array("FontSize"=>20,"Align"=>TEXT_ALIGN_BOTTOMRIGHT)); 
