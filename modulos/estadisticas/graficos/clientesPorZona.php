<?php    
	...
	$query="SELECT charValue FROM config WHERE descConfig = 'zona' AND charValue != 'Pruebas'";
	$result=mysqli_query($dbcon, $query) or die(mysqli_error($dbcon));
	$zonas_cargadas = array();

	while($fila = mysqli_fetch_array($result, MYSQLI_NUM)){
		array_push($zonas_cargadas, $fila[0]);
	}

	$query="SELECT  clientes.idZona AS idz,
					COUNT(clientes.idCliente) AS cant,
					config.charValue AS zona					
					FROM clientes
					INNER JOIN config ON clientes.idZona = config.idConf
					GROUP BY idz";
	$result=mysqli_query($dbcon, $query) or die(mysqli_error($dbcon));
	...
	while($fila = mysqli_fetch_array($result, MYSQLI_ASSOC)){
		if(array_search($fila['zona'], $zonas_cargadas) >= 0){
			array_push($cants, $fila['cant']);
			array_push($zonas, $fila['zona']);
	...
	$myPicture = new pImage(800,420,$MyData);
	
	/* Draw the background */ 
	$Settings = array("R"=>170, "G"=>183, "B"=>87, "Dash"=>1, "DashR"=>190, "DashG"=>203, "DashB"=>107); 
	$myPicture->drawFilledRectangle(0,0,800,420,$Settings); 

	/* Overlay with a gradient */ 
	$Settings = array("StartR"=>219, "StartG"=>231, "StartB"=>139, "EndR"=>1, "EndG"=>138, "EndB"=>68, "Alpha"=>50); 
	$myPicture->drawGradientArea(0,0,800,420,DIRECTION_VERTICAL,$Settings); 

	/* Add a border to the picture */ 
	$myPicture->drawRectangle(0,0,799,419,array("R"=>0,"G"=>0,"B"=>0)); 
	
	/* Write the chart title */  
	$myPicture->setFontProperties(array("FontName"=>FONT_PATH."/Forgotte.ttf","FontSize"=>15)); 
	$myPicture->drawText(200,37,"Clientes por zona",array("FontSize"=>20,"Align"=>TEXT_ALIGN_BOTTOMRIGHT)); 

	/* Define the default font */  
	$myPicture->setFontProperties(array("FontName"=>FONT_PATH."/pf_arma_five.ttf","FontSize"=>10)); 

	/* Set the graph area */  
	$myPicture->setGraphArea(160,60,730,400); 
	$myPicture->drawGradientArea(160,60,730,400,DIRECTION_HORIZONTAL,array("StartR"=>200,"StartG"=>200,"StartB"=>200,"EndR"=>255,"EndG"=>255,"EndB"=>255,"Alpha"=>30)); 
